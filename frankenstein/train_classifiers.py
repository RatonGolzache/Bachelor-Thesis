...
from model_encoder import ClassifierSpk, ClassifierContent, ClassifierEmo, SpeakerEncoder, EmoEncoder
...
def save_checkpoint(classifier_emo, classifier_spk, classifier_content, scheduler, amp, epoch,
    ...
    checkpoint_state = {
        ...
        #"spk_classifier_spk": spk_classifier_spk.state_dict(),
        #"spk_classifier_content": spk_classifier_content.state_dict(),
        #"spk_classifier_emo": spk_classifier_emo.state_dict(),
...
def train_model(cfg):
    ...
    checkpoint_dir = Path(utils.to_absolute_path("checkpoint_spk_classifiers_adv1_cls_cremad"))
    checkpoint_dir.mkdir(exist_ok=True, parents=True)
    ...
    classifier_spk = ClassifierSpk(256, 128, cfg.classifier_num_speakers_cremad) # classifier from speaker embedding
    classifier_content = ClassifierContent(64, 32, cfg.classifier_num_speakers_cremad) # classifier from content embedding
    classifier_emo = ClassifierEmo(256, 128, cfg.classifier_num_speakers_cremad) # classifier from style embedding
    ...
    #spk_classifier_spk.to(device)
    #spk_classifier_content.to(device)
    #spk_classifier_emo.to(device)
    ...
    for epoch in range(start_epoch, cfg.training.n_epochs + 1):
        ...
        for i, (labels, mels, lf0, speakers) in enumerate(dataloader, 1):
            ...
            speakers = speakers.to(device)  # (bs,)
