...
from model_encoder import SpeakerEncoder, EmoEncoder, GroupCenterLoss, EmotionClassifier, SpeakerClassifier
...
def forward(mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, decoder, cfg, optimizer, scheduler, 
            labels, speakers, group_center_loss_emo, group_center_loss_spk, emo_classifier, spk_classifier):
    optimizer.zero_grad()
    ...
    loss = recon_loss + vq_loss + cpc_loss + center_loss_emo + center_loss_spk + cross_entropy_loss + cross_entropy_loss_spk
    ...
    optimizer.step()
    return optimizer, recon_loss, cpc_loss, vq_loss, accuracy, perplexity, center_loss_emo, center_loss_spk, cross_entropy_loss, cross_entropy_loss_spk
...
def train_model(cfg):
    ...
    optimizer = optim.Adam(
        chain(encoder.parameters(), encoder_lf0.parameters(), cpc.parameters(), encoder_spk.parameters(),
              encoder_emo.parameters(), decoder.parameters(), group_center_loss_emo.parameters(), 
              group_center_loss_spk.parameters(), emo_classifier.parameters(), spk_classifier.parameters()),
    ...
    if cfg.use_amp:
        [encoder, encoder_lf0, cpc, encoder_spk, decoder, group_center_loss_emo, group_center_loss_spk, emo_classifier, spk_classifier], optimizer = amp.initialize(
            [encoder, encoder_lf0, cpc, encoder_spk, decoder, group_center_loss_emo, group_center_loss_spk, emo_classifier, spk_classifier], optimizer, opt_level='O1')
    ...
    for epoch in range(start_epoch, cfg.training.n_epochs + 1):
        ...
        for i, (labels, mels, lf0, speakers) in enumerate(dataloader, 1):
            ...
            optimizer, recon_loss, cpc_loss, vq_loss, accuracy, perplexity, emo_center_loss, spk_center_loss, cross_entropy_loss_emo, cross_entropy_loss_spk = forward(
                mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, decoder, 
                cfg, optimizer, scheduler, labels, speakers, group_center_loss_emo, group_center_loss_spk, emo_classifier, spk_classifier)
            ...
            if (global_step % 25 == 0) and (global_step != 0):
                print(
                    "epoch:{}, global step:{}, cross entropy loss:{:.3f}, cross entropy loss speaker:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, emotion group center loss:{:.8f}, speaker group center loss:{:.8f}, used time:{:.3f}s"
        ...
        results_txt.write(
            "epoch:{}, global step:{}, cross entropy loss:{:.3f}, cross entropy loss speaker:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, emotion group center loss:{:.8f}, speaker group center loss:{:.8f}"
            .format(epoch, global_step, average_cross_entropy_loss_emo, average_cross_entropy_loss_spk, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity, average_emo_center_loss, average_spk_center_loss) + '\n')
