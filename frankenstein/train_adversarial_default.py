...
from model_encoder import SpeakerEncoder, EmoEncoder, SimpleAdversarialClassifier, GeneralizedCrossEntropyLoss
...
def check_tensor(tensor, name):
    if torch.isnan(tensor).any():
        print(f"Warning: NaN detected in {name}, shape: {tensor.shape}")
    if torch.isinf(tensor).any():
        print(f"Warning: Inf detected in {name}, shape: {tensor.shape}")

def check_model_numerics(models):
    for model in models:
        for name, param in model.named_parameters():
            if param.grad is not None:
                if torch.isnan(param.grad).any():
                    print(f"NaN detected in gradient of {name} in {model.__class__.__name__}")
                if torch.isinf(param.grad).any():
                    print(f"Inf detected in gradient of {name} in {model.__class__.__name__}")
            if torch.isnan(param).any():
                print(f"NaN detected in parameter {name} in {model.__class__.__name__}")
            if torch.isinf(param).any():
                print(f"Inf detected in parameter {name} in {model.__class__.__name__}")
...
def forward(mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, decoder, cfg, optimizer, scheduler, 
            labels, speakers, adversarial_emo_classifier, adversarial_spk_classifier, criterion):
    
    # Usage example after loss.backward(), before optimizer.step():
    models = [encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, decoder,
          adversarial_emo_classifier, adversarial_spk_classifier]
    ...
    #check_tensor(z, "z (content embedding)")
    #check_tensor(c, "c (context embedding)")   
    cpc_loss, accuracy = cpc(z, c)
    lf0_embs = encoder_lf0(lf0)
    c#heck_tensor(lf0_embs, "lf0_embs")    
    spk_embs = encoder_spk(mels)
    #check_tensor(spk_embs, "spk_embs")
    emo_embs = encoder_emo(mels)
    #check_tensor(emo_embs, "emo_embs")


    # Emotion embeddings passed to adversarial speaker classifier
    adv_spk_logits = adversarial_spk_classifier(emo_embs)
    #check_tensor(adv_spk_logits, "adv_spk_logits (emotion embeddings -> speaker classifier)")
    ...
    #check_tensor(adv_emo_logits, "adv_emo_logits (speaker embeddings -> emotion classifier)")
    spk_adv_loss = criterion(adv_emo_logits, labels)

    #adv_spk_logits.register_hook(lambda grad: print('adv_spk grad:', grad))
    #adv_emo_logits.register_hook(lambda grad: print('adv_emo grad:', grad))


    recon_loss, pred_mels = decoder(z, lf0_embs, spk_embs, emo_embs, mels.transpose(1, 2))
    #check_tensor(pred_mels, "pred_mels")

    loss = recon_loss + vq_loss + cpc_loss + emo_adv_loss + spk_adv_loss
    #if torch.isnan(loss) or torch.isinf(loss):
    #    print(f"Warning: NaN or Inf detected in total loss: {loss}")
    ...
    #check_model_numerics(models)
        
    optimizer.step()
    return optimizer, recon_loss, cpc_loss, vq_loss, accuracy, perplexity, emo_adv_loss, spk_adv_loss
...
def train_model(cfg):
    ...
    adversarial_emo_classifier = SimpleAdversarialClassifier(cfg.classifier_input_dim, cfg.classifier_num_classes, 100.0)
    adversarial_spk_classifier = SimpleAdversarialClassifier(cfg.classifier_input_dim, cfg.classifier_num_speakers, 100.0)
    ...
    if cfg.use_amp:
        [encoder, encoder_lf0, cpc, encoder_spk, decoder, adversarial_emo_classifier, adversarial_spk_classifier], optimizer = amp.initialize(
            [encoder, encoder_lf0, cpc, encoder_spk, decoder, adversarial_emo_classifier, adversarial_spk_classifier], optimizer, opt_level='O1')
    ...
    for epoch in range(start_epoch, cfg.training.n_epochs + 1):
        ...
        for i, (labels, mels, lf0, speakers) in enumerate(dataloader, 1):
            ...
            optimizer, recon_loss, cpc_loss, vq_loss, accuracy, perplexity, emo_adv_loss, spk_adv_loss = forward(
                mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, decoder, 
                cfg, optimizer, scheduler, labels, speakers, adversarial_emo_classifier, adversarial_spk_classifier, criterion)
            ...
            if (global_step % 25 == 0) and (global_step != 0):
                print(
                    "epoch:{}, global step:{}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, emotion adversarial loss:{:.8f}, speaker adversarial loss:{:.8f}, used time:{:.3f}s"
        ...
        results_txt.write(
            "epoch:{}, global step:{}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, emotion adversarial loss:{:.8f}, speaker adversarial loss:{:.8f}"
            .format(epoch, global_step, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity, average_emo_adv_loss, average_spk_adv_loss) + '\n')
