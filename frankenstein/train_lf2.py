...
from model_encoder import EmotionClassifier, SpeakerClassifier
...
# amp state_dict. 
def save_checkpoint(encoder, encoder_lf0, encoder_emo, cpc, encoder_spk, decoder, \
                    optimizer, scheduler, amp, epoch,
                    checkpoint_dir, cfg, emo_classifier, spk_classifier):
    if cfg.use_amp:
...
def label_forward(mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, decoder, cfg, optimizer, scheduler, emo_classifier, labels, spk_classifier, speakers):
    optimizer.zero_grad()
    ...
    loss = recon_loss + cross_entropy_loss + cross_entropy_loss_spk + cpc_loss + vq_loss
    ...
    optimizer.step()
    return optimizer, recon_loss, cross_entropy_loss, cross_entropy_loss_spk, cpc_loss, vq_loss, accuracy, perplexity
...
# Train the model using the MI estimators and the other losses
# This function performs a forward pass through the model, computes the losses, and updates the model parameters.
def mi_second_forward(mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, cs_mi_net, ps_mi_net, cp_mi_net,ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, optimizer, scheduler, emo_classifier, labels, spk_classifier, speakers):
    optimizer.zero_grad()
    # z = content embedding, c = context embedding (dependencies across time steps)
    z, c, _, vq_loss, perplexity = encoder(mels)
    cpc_loss, accuracy = cpc(z, c)
...
def calculate_eval_loss(mels, lf0, \
                        ...
                        cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, emo_classifier, labels, spk_classifier, speakers):
    with torch.no_grad():
        ...
        return recon_loss, vq_loss, cpc_loss, accuracy, perplexity, mi_cs_loss, lld_cs_loss, mi_ps_loss, lld_ps_loss, \
               mi_cp_loss, lld_cp_loss, mi_ec_loss, lld_ec_loss, mi_se_loss, lld_se_loss, mi_ep_loss, lld_ep_loss, cross_entropy_loss, cross_entropy_loss_spk
...
def eval_model(epoch, checkpoint_dir, device, valid_dataloader, encoder, encoder_lf0, encoder_emo, cpc, encoder_spk,
               cs_mi_net, ps_mi_net, cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, emo_classifier, spk_classifier):
    stime = time.time()
    average_cpc_loss = average_vq_loss = average_perplexity = average_recon_loss = average_cross_entropy_loss = average_cross_entropy_loss_spk = 0
    ...
    all_models = [encoder, encoder_lf0, cpc, encoder_spk, encoder_emo, cs_mi_net, ps_mi_net, cp_mi_net, ec_mi_net,
                  se_mi_net, ep_mi_net, decoder, emo_classifier, spk_classifier]
    to_eval(all_models)
    ...
    ctime = time.time()
    print(
        "Eval | epoch:{}, cross entropy loss:{:.3f}, spk cross entropy loss:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, lld cs loss:{:.3f}, mi cs loss:{:.3E}, lld ps loss:{:.3f}, mi ps loss:{:.3f}, lld cp loss:{:.3f}, mi cp loss:{:.3f}, used time:{:.3f}s"
            .format(epoch, average_cross_entropy_loss, average_cross_entropy_loss_spk, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity,
                    average_lld_cs_loss,
    ...
    results_txt.write(
        "Eval | epoch:{}, cross entropy loss:{:.3f}, spk cross entropy loss:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, lld cs loss:{:.3f}, mi cs loss:{:.3E}, lld ps loss:{:.3f}, mi ps loss:{:.3f}, lld cp loss:{:.3f}, mi cp loss:{:.3f}"
        .format(epoch, average_cross_entropy_loss, average_cross_entropy_loss_spk, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity, average_lld_cs_loss,
...
def train_model(cfg):
    ...
    emo_classifier = EmotionClassifier(cfg.classifier_input_dim, cfg.classifier_num_classes_cremad) # emotion classifier
    spk_classifier = SpeakerClassifier(cfg.classifier_input_dim, cfg.classifier_num_speakers_cremad) # speaker classifier
    ...
    optimizer = optim.Adam(
        chain(encoder.parameters(), encoder_lf0.parameters(), cpc.parameters(), encoder_spk.parameters(),
              encoder_emo.parameters(), decoder.parameters(), emo_classifier.parameters(), spk_classifier.parameters()),
    ...
    if cfg.use_amp:
        [encoder, encoder_lf0, cpc, encoder_spk, decoder, emo_classifier, spk_classifier], optimizer = amp.initialize(
            [encoder, encoder_lf0, cpc, encoder_spk, decoder, emo_classifier, spk_classifier], optimizer, opt_level='O1')
    ...
    for epoch in range(start_epoch, cfg.training.n_epochs + 1):
        ...
        for i, (labels, mels, lf0, speakers) in enumerate(dataloader, 1):
            ...
            optimizer, recon_loss, cross_entropy_loss, cross_entropy_loss_spk, cpc_loss, vq_loss, accuracy, perplexity = label_forward(
                mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, decoder, 
                cfg, optimizer, scheduler, emo_classifier, labels, spk_classifier, speakers)
            ...
            mi_ps_loss, mi_cp_loss, mi_ec_loss, mi_se_loss, mi_ep_loss = mi_second_forward(
                ...
                ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, optimizer, scheduler, emo_classifier, labels, spk_classifier, speakers)
            ...
            if (global_step % 25 == 0) and (global_step != 0):
                print(
                    "epoch:{}, global step:{}, cross entropy loss:{:.3f}, cross entropy loss speaker:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, lld cs loss:{:.3f}, mi cs loss:{:.3E}, lld ps loss:{:.3f}, mi ps loss:{:.3f}, lld cp loss:{:.3f}, mi cp loss:{:.3f}, used time:{:.3f}s"
                        .format(epoch, global_step, average_cross_entropy_loss, average_cross_entropy_loss_spk, average_recon_loss, average_cpc_loss, average_vq_loss,
                                average_perplexity,
            ...
            #if (global_step % 25 == 0) and (global_step != 0):
            #    print(
            #        "epoch:{}, global step:{}, cross entropy loss:{:.3f}, cross entropy loss speaker:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, used time:{:.3f}s"
            #            .format(epoch, global_step, average_cross_entropy_loss, average_cross_entropy_loss_spk, average_recon_loss, average_cpc_loss, average_vq_loss,
            #                    average_perplexity, ctime - stime))
            #    print(100 * average_accuracies)
            stime = time.time()
            global_step += 1
            # scheduler.step()
        ...
        results_txt = open(f'{str(checkpoint_dir)}/results.txt', 'a')
        results_txt.write(
            "epoch:{}, global step:{}, cross entropy loss:{:.3f}, cross entropy loss speaker:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, lld cs loss:{:.3f}, mi cs loss:{:.3E}, lld ps loss:{:.3f}, mi ps loss:{:.3f}, lld cp loss:{:.3f}, mi cp loss:{:.3f}"
            .format(epoch, global_step, average_cross_entropy_loss, average_cross_entropy_loss_spk, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity,
                    average_lld_cs_loss, average_mi_cs_loss, average_lld_ps_loss, average_mi_ps_loss,
                    average_lld_cp_loss, average_mi_cp_loss, average_lld_ec_loss, average_mi_ec_loss, \
                    average_lld_se_loss, average_mi_se_loss, average_lld_ep_loss, average_mi_ep_loss) + '\n')
        #results_txt.write(
        #    "epoch:{}, global step:{}, cross entropy loss:{:.3f}, cross entropy loss speaker:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}"
        #    .format(epoch, global_step, average_cross_entropy_loss, average_cross_entropy_loss_spk, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity) + '\n')
        #results_txt.write(' '.join([str(cpc_acc) for cpc_acc in average_accuracies]) + '\n')
        results_txt.close()
        scheduler.step()
        ...
        #               encoder_spk, cs_mi_net, ps_mi_net, cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, emo_classifier, spk_classifier)
        #
        #    ctime = time.time()
        #    print(
        #        "epoch:{}, global step:{}, cross entropy loss:{:.3f}, cross entropy loss speaker:{:.3f}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, lld cs loss:{:.3f}, mi cs loss:{:.3E}, lld ps loss:{:.3f}, mi ps loss:{:.3f}, lld cp loss:{:.3f}, mi cp loss:{:.3f}, used time:{:.3f}s"
