...
def save_checkpoint(encoder, encoder_lf0, encoder_emo, cpc, encoder_spk, \
                    ...
                    checkpoint_dir, cfg, emo_classifier):
    if cfg.use_amp:
...
def mi_second_forward(mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, cs_mi_net, ps_mi_net, cp_mi_net,ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, optimizer, scheduler, emo_classifier, labels):
    optimizer.zero_grad()
...
def calculate_eval_loss(mels, lf0, \
                        ...
                        cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, emo_classifier, labels):
    with torch.no_grad():
...
def eval_model(epoch, checkpoint_dir, device, valid_dataloader, encoder, encoder_lf0, encoder_emo, cpc, encoder_spk,
               cs_mi_net, ps_mi_net, cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, emo_classifier):
    stime = time.time()
    ...
    all_models = [encoder, encoder_lf0, cpc, encoder_spk, encoder_emo, cs_mi_net, ps_mi_net, cp_mi_net, ec_mi_net,
                  se_mi_net, ep_mi_net, decoder, emo_classifier]
    to_eval(all_models)
...
def train_model(cfg):
    ...
    optimizer = optim.Adam(
        chain(encoder.parameters(), encoder_lf0.parameters(), cpc.parameters(), encoder_spk.parameters(),
              encoder_emo.parameters(), decoder.parameters(), emo_classifier.parameters()),
    ...
    for epoch in range(start_epoch, cfg.training.n_epochs + 1):
        ...
        for i, (labels, mels, lf0, speakers) in enumerate(dataloader, 1):
            ...
            mi_ps_loss, mi_cp_loss, mi_ec_loss, mi_se_loss, mi_ep_loss, cross_entropy_loss = mi_second_forward(
                ...
                ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, optimizer, scheduler, emo_classifier, labels)
        ...
        if epoch % cfg.training.log_interval == 0 and epoch != start_epoch:
            eval_model(epoch, checkpoint_dir, device, valid_dataloader, encoder, encoder_lf0, encoder_emo, cpc,
                       encoder_spk, cs_mi_net, ps_mi_net, cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, emo_classifier)
